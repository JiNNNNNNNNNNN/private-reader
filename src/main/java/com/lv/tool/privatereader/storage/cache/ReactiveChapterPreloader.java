...
public final class ReactiveChapterPreloader {
    ...
    public Mono<Void> preloadChaptersReactive(Book book, int currentChapterIndex) {
        return Mono.defer(() -> {
            ...
            LOG.info("开始响应式预加载后续章节，当前章节索引: " + currentChapterIndex);
            ...
            // 创建要预加载的章节范围
            return Flux.range(currentChapterIndex + 1, endIndex - currentChapterIndex)
                // 检查是否仍在预加载状态
                ...
                // 完成后记录日志
                .doOnComplete(() -> LOG.info("章节预加载完成，预加载范围: " + (currentChapterIndex + 1) + " - " + endIndex))
                // 错误处理
